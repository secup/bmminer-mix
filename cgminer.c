...
static struct timeval total_tv_end;
static struct timeval restart_tv_start;
static struct timeval update_tv_start;
...
static struct opt_table opt_config_table[] =
{
    ...
    set_int_0_to_100, opt_show_intval, &opt_bitmain_fan_max,
    "Set bitmain fan 35 to 100"),

    OPT_WITH_ARG("--bitmain-fan-min",
    set_int_0_to_100, opt_show_intval, &opt_bitmain_fan_min,
    "Set bitmain fan 35 to 100"),
...
static struct opt_table opt_cmdline_table[] =
{
    ...
    OPT_WITHOUT_ARG("--help|-h", opt_verusage_and_exit, NULL,
    "Print this message"),
...
static struct work *make_work(void)
{
    ...
    work->id = (uint32_t) total_work_inc();
...
void _free_work(struct work **workptr, const char *file, const char *func, const int line)
{
    ...
    if (unlikely(!work))
    {
        applog(LOG_ERR, "Free work called with NULL work from %s %s:%d", file, func, line);
...
static void update_gbt(struct pool *pool)
{
    ...
    if (val)
    {
        struct work *work = make_work(); // update_gbt
...
static void get_statline(char *buf, size_t bufsiz, struct cgpu_info *cgpu)
{
    ...
    dh64 = cgpu->total_mhashes / dev_runtime * 1000000ull;
    dr64 = cgpu->rolling * 1000000ull;
...
static void curses_print_status(void)
{
    ...
    if (opt_widescreen)
    {
        cg_mvwprintw(statuswin, 3, 0, " A:%.0f  R:%.0f  HW:%d  WU:%.1f/m | ST: %d  SS: %"PRId64"  NB: %d  LW: %d  GF: %d  RF: %d",
...
static void curses_print_devstatus(struct cgpu_info *cgpu, int devno, int count)
{
    ...
    if (cgpu->usbinfo.nodev)
        ...
        else if (opt_widescreen)
        {
            ...
            d64 = cgpu->total_mhashes / dev_runtime * 1000000ull;
            suffix_string(d64, displayed_hashes, sizeof(displayed_hashes), 4);
            d64 = cgpu->rolling * 1000000ull;
...
static bool submit_upstream_work(struct work *work, CURL *curl, bool resubmit)
{
    ...
    if (!QUIET)
    {
        ...
        if (opt_worktime)
        {
            ...
            double getwork_time = tdiff(&(work->tv_getwork_reply),
                                        &(work->tv_getwork));
            double getwork_to_work = tdiff(&(work->tv_work_start),
                                           &(work->tv_getwork_reply));
            double work_time = tdiff(&(work->tv_work_found),
                                     &(work->tv_work_start));
...
{
    struct work *work = make_work(); // *copy_work_noffset
...
static bool test_work_current(struct work *work)
{
    ...
    if (cb_height_sz <= 4)
    {
        cg_memcpy(&height, bin_height, cb_height_sz);
    ...
    if (!block_exists(hexstr, bedata, work))
    {
        ...
        if (work->longpoll)
        {
            if (work->stratum)
            {
                applog(LOG_NOTICE, "Stratum from pool %d detected new block at height %d", pool->pool_no, height);
            }
            else
            {
...
//    total_secs = tdiff(&total_tv_end, &total_tv_start);
    ...
    if (showlog)
    {
        ...
        d64 = total_mhashes_done / total_secs * 1000000ull;
        suffix_string(d64, displayed_hashes, sizeof(displayed_hashes), 4);

        d64 = total_rolling * 1000000ull;
        g_displayed_rolling = ((((int)total_rolling)/10)*1.0) / 100.0;  // only keep 2 numbers after dot

        suffix_string(d64, displayed_rolling, sizeof(displayed_rolling), 4);
        d64 = rolling1 * 1000000ull;

        suffix_string(d64, displayed_r1, sizeof(displayed_rolling), 4);
        d64 = rolling5 * 1000000ull;

        suffix_string(d64, displayed_r5, sizeof(displayed_rolling), 4);
        d64 = rolling15 * 1000000ull;
...
static void stratum_share_result(json_t *val, json_t *res_val, json_t *err_val, struct stratum_share *sshare)
{
    ...
    {
        applog(LOG_INFO, "Pool %d stratum share result lag time %d seconds", work->pool->pool_no, srdiff);
    }
...
retry_stratum:
    ...
    if (!pool->probed)
    {
        ...
        val = json_rpc_call(curl, pool->rpc_url, pool->rpc_userpass, gbt_req, true, false, &rolltime, pool, false);
    ...
    cgtime(&tv_getwork);
    val = json_rpc_call(curl, pool->rpc_url, pool->rpc_userpass, pool->rpc_req, true, false, &rolltime, pool, false);
    ...
    if (val)
    {
        ...
        {
            applog(LOG_DEBUG, "Successfully retrieved but FAILED to decipher work from pool %u %s", pool->pool_no, pool->rpc_url);
            free_work(work);
        }
...
static void gen_hash(unsigned char *data, unsigned char *hash, int len)
{
    ...
    sha256(data, (unsigned int) len, hash1);
...
void set_target(unsigned char *dest_target, double diff)
{
    ...
    *data64  = htole64(h64);
    ...
    if (opt_debug)
    {
        char *htarget = bin2hex(target, (size_t)32);
...
static void gen_stratum_work(struct pool *pool, struct work *work)
{
    ...
    if (opt_debug)
    {
        ...
        header = bin2hex(work->data, (size_t)112);
        merkle_hash = bin2hex((const unsigned char *)merkle_root, (size_t)32);
...
static void submit_work_async(struct work *work)
{
    ...
    if (opt_benchmark)
    {
        ...
        applog(LOG_NOTICE, "Accepted %s %d benchmark share nonce %08x", cgpu->drv->name, cgpu->device_id, *(uint32_t *)(work->data + 64 + 12));
        return;
    }
...
bool submit_tested_work(struct thr_info *thr, struct work *work)
{
    ...
    {
        applog(LOG_INFO, "%s %s %d: Share above target",__FUNCTION__, thr->cgpu->drv->name, thr->cgpu->device_id);
        return false;
    }
...
struct work *__get_queued(struct cgpu_info *cgpu)
{
    ...
    if (cgpu->unqueued_work)
    {
        ...
        {
            discard_work(work); // *__get_queued
            wake_gws();
        }
        else
        {
...
// TODO: kano what do you think about this?

        if (mythr->work_update)
        {
            mutex_lock(&update_job_lock);
            drv->update_work(cgpu);
            mutex_unlock(&update_job_lock);
        }
    }
    cgpu->deven = DEV_DISABLED;
}
...
static void *watchpool_thread(void __maybe_unused *userdata)
{
    ...
    while (42)
    {
        ...
        for (i = 0; i < total_pools; i++)
        {
            ...
            {
                applog(LOG_WARNING, "Pool %d %s stable for >%d seconds", pool->pool_no, pool->rpc_url, opt_pool_fallback);
                switch_pools(NULL);
            }
        }
...
static void *watchdog_thread(void __maybe_unused *userdata)
{
    ...
    while (1)
    {
        ...
        {
            applog(LOG_WARNING, "Pausing execution as per stop time %02d:%02d scheduled", schedstop.tm.tm_hour, schedstop.tm.tm_min);
            ...
            applog(LOG_WARNING, "Will restart execution as scheduled at %02d:%02d", schedstart.tm.tm_hour, schedstart.tm.tm_min);
            sched_paused = true;
        ...
        {
            applog(LOG_WARNING, "Restarting execution as per start time %02d:%02d scheduled", schedstart.tm.tm_hour, schedstart.tm.tm_min);

            if (schedstop.enable)
            {
                applog(LOG_WARNING, "Will pause execution as scheduled at %02d:%02d", schedstop.tm.tm_hour, schedstop.tm.tm_min);
            }
...
void fill_device_drv(struct device_drv *drv)
{
    ...
    if (!drv->get_stats)
    {
        drv->get_stats = (bool (*)(struct cgpu_info *)) &noop_get_stats;
    }

    if (!drv->thread_prepare)
    {
        drv->thread_prepare = (bool (*)(struct thr_info *)) &noop_thread_prepare;
    }
    ...
    if (!drv->thread_init)
    {
        drv->thread_init = (bool (*)(struct thr_info *)) &noop_thread_init;
    }

    if (!drv->prepare_work)
    {
        drv->prepare_work = (bool (*)(struct thr_info *, struct work *)) &noop_prepare_work;
    }
...
void null_device_drv(struct device_drv *drv)
{
    ...
    drv->set_device          = NULL;
    drv->thread_prepare      = (bool (*)(struct thr_info *)) &noop_thread_prepare;
    drv->can_limit_work      = (uint32_t (*)(struct thr_info *)) &noop_can_limit_work;
    drv->thread_init         = (bool (*)(struct thr_info *)) &noop_thread_init;
    drv->prepare_work        = (bool (*)(struct thr_info *, struct work *)) &noop_prepare_work;
...
# ifdef __linux
    ...
    if (opt_benchmark || opt_benchfile)
    {
        ...
        pool = add_pool();
        pool->rpc_url = cgmalloc((size_t)255);
